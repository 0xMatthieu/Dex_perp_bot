...
<section class="wide"><h2>Last trades <span class="muted" id="ctsum"></span></h2><div class="overflow"><table id="closed"></table></div><div class="overflow" style="margin-top:8px"><table id="trades"></table></div></section>
...
let refresh=async function(){
  ...
  $('#opps').innerHTML=op.length?'<tr><th>Symbol</th><th>Long</th><th>Short</th><th class=num>Next-hour APY</th><th class=num>Steady APY</th><th>Basis</th><th>Imminent</th><th>Actionable</th><th>Gate</th><th class=num>Aster rate</th><th class=num>HL rate</th></tr>'+
    op.map(o=>`<tr><td>${esc(o.symbol)}</td><td>${venueTag(o.long_venue)}</td><td>${venueTag(o.short_venue)}</td><td class="num ${cls(o.net_apy)}">${o.net_apy.toFixed(1)}%</td><td class="num ${cls(o.apy_steady)}">${o.apy_steady!=null?o.apy_steady.toFixed(1)+'%':'—'}</td><td>${o.basis}</td><td>${o.imminent?'yes':''}</td><td>${o.actionable?'yes':'<span class=neg>no</span>'}</td><td class="${o.gate==='selected'?'pos':(o.gate&&o.gate!=='not_evaluated'?'neg':'muted')}">${esc(o.gate||'—')}</td><td class=num>${(o.rate_aster*100).toFixed(4)}%</td><td class=num>${(o.rate_hyperliquid*100).toFixed(4)}%</td></tr>`).join('')
    :'<tr><td class=muted>No scan yet (first scan happens in the next trading window)</td></tr>';
  const ct=st.closed_trades||[];const cs=ct.reduce((a,t)=>({n:a.n+t.net,f:a.f+t.funding,x:a.x+t.fees,p:a.p+t.trading}),{n:0,f:0,x:0,p:0});
  $('#ctsum').innerHTML=ct.length?`· ${ct.length} closed · net <span class="${cls(cs.n)}">${usd(cs.n)}</span> = price ${usd(cs.p)} + fees ${usd(cs.x)} + funding ${usd(cs.f)}`:'';
  const tm=ms=>{const d=new Date(ms);return d.toLocaleDateString(undefined,{month:'2-digit',day:'2-digit'})+' '+d.toLocaleTimeString(undefined,{hour:'2-digit',minute:'2-digit'})};
  $('#closed').innerHTML=ct.length?'<tr><th>Closed</th><th>Symbol</th><th>Venues closed</th><th class=num>Held</th><th class=num title="Hyperliquid leg notional at exit">Notional (HL)</th><th class=num title="Realized price P&L, both venues">Price</th><th class=num>Fees</th><th class=num>Funding</th><th class=num>Net PnL</th></tr>'+
    ct.map(t=>`<tr><td title="opened ${tm(t.opened_ms)}">${tm(t.closed_ms)}</td><td>${esc(t.symbol)}</td><td>${t.venues.map(v=>venueTag(v==='hyperliquid'?'Hyperliquid':'Aster')).join(' ')}${t.venues.length<2?' <span class=neg title="only one venue closed in this exit: unhedged leg">single leg</span>':''}</td><td class=num>${t.hours.toFixed(1)}h</td><td class=num>${t.close_notional?usd(t.close_notional):'—'}</td><td class="num ${cls(t.trading)}">${usd(t.trading)}</td><td class="num ${cls(t.fees)}">${usd(t.fees)}</td><td class="num ${cls(t.funding)}">${usd(t.funding)}</td><td class="num ${cls(t.net)}" style="font-weight:600">${usd(t.net)}</td></tr>`).join('')
    :'<tr><td class=muted>No closed trades in the last 30 days (P&amp;L comes from venue fills, next status refresh)</td></tr>';
